...
To create graphical and numerical summaries of two categorical variables

1. Bar chart expectations
2. Contingency table review
3. Dropping levels
4. Side-by-side barcharts
5. Bar chart interpretation
6. counts vs. proportions
7. conditional proportions
8. distribution of one variable
9. marginal barchart
10. conditional barchart
11. improve piechart

## EDA: Numerical Data

To graphically summarize numerical data

1. Faceted histogram
2. Boxplots and density plots
3. Compare distribution via plots
4. Distribution of one variable
5. Marginal and conditional histograms
6. Margincal and conditional histograms interpretation
7. Three binwidths
8. Three binwidths interpretation
9. Box plots
10. Box plots for outliers
11. Plot selection
12. Visualization in higher dimensions
13. 3 variable plot
14. Interpret 3 var plot

## Numerical Summaries: Measures of center and Measures of spread

Some useful statistics for describing distribution of data

1. Measures of center
2. Choice of center measure
3. Calculate center measures
4. Measures of variability
5. Choice of spread measure
6. Calculate spread measures
7. Choose measures for center and spread
8. Shape and transformations
...
To see combination of levels for two categorical variable use contingency table

```{r}
table(comics$id, comics$align)
```

The output tells us the most common category had a count of 4493 characters with Secret identities.

While tables of counts are useful you can get a bigger picture by translating these counts into graphics

```{r}
# x-axis id is factor variable, so bar chart fill by another categorical variable align
...
comics dataset is collection of characteristics on all the superheroes created by Marvel and DC comics in the last 80 years.
...
have even just 1 or 2 counts
- it only drops levels that have no count
...
This shows that there can often be two or more options for presenting the same data.
Passing the argument position = 'dodge' to geom_bar() says that you want a side-by-side (i.e. not stacked) barchart.

side-by-sie indicates separate bar for each category { y axis has the frequency }
...
interpretation: 
so we can see that 57 % of all secret characters are Bad 31% are good and 12% are neutral
...
Now columns sum to 1 and we learned 63 % of Bad characters secret
...
Note: In proportion all bars are same height and fill category corresponds to the proportion 

**Counts vs. proportions**

Bars charts can tell dramatically different stories depending on whether they represent counts or proportions and, if proportions, what the proportions are conditioned on.
...
Create a stacked barchart of gender proportions with alignon the x-axis by setting the position argument to geom_bar() equal to 'fill'.
...
474 + 647 + 390 = 1511
Since we have summed over the margins of other variables this is sometimes known as marginal distribution
...
We could filter the dataset to include only neutral characters or we could use a technique called faceting Faceting breaks the data into subsets based on a categorical variable and then constructs a plot for each facet_wrap(~align) 
~ implying broken down by align, that is character
...
Superheroes that are 'Neutral' shown an alignment between Good and Bad so it makes sense to put that bar in the middle.
...
Most common way to plot numerical data is dotplot where each case is plotted as a dot on the x-axis
...
In Dotplot there is zero information loss and the dataset can be rebuilt from it, but it gets difficult to read dotplots as the number of cases gets very large.
To solve this problem use histograms which aggregate the plot into bins on the x-axis and height of the bin corresponding to the number of cases that fall in that bin
...
Due to the binning it is not possible to perfectly reconstruct the dataset from histograms but what we gain is the bigger picture of the distribution of the data If the step-wise structure of the histogram irks you then you will like the Density plot.
...
x-axis numerical variable weight, boxplot shows summary stats along y-axis so set x to 1
...
Another scenario would be to see the distribution on a subset of the data
...
- {marginal is for one variable conditional means subsetted dataset}
...
Create a second histogram of the distribution of horsepwer, but only for those cars that have an msrp less than $25000 
Keep the limits of the x-axis so that they're similar to that of the first plot, and a descriptive title.
...
Nice! Be sure to toggle back and forth in the plots pane to compare the histograms.
...
- first, second and third quartile of the data
...
# Exploratory Data Analysis of life dataset
...
Note: Imp
Here both, median and mode are same. But thats often not the case.
...
#Using gap2007 calculate the mean and median life expectancy for each continent. 
...
# Confirm the trends that you see in the medians by generating side-by-side box plots
...
Income in west coast counties is indeed greater than in the rest of the country
...
create outlier income indicator for any threshold income, say 45000 usd
Below mutate assign true to the new variable if condition is true else false
